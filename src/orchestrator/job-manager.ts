...
export class JobManager {
  ...
  private async executeResearch(job: Job) {
    ...
    if (plan.mode === "article_research" && !hasUsableResearchMaterial(input.text, input.urls)) {
      return {
        ok: true,
        summary:
          "当前群已配置为文章研究。请先发送文章链接，或粘贴足够完整的正文内容（建议不少于200字），我再基于材料做研究分析。",
        metadata: {
          jobId: job.id,
          mode: plan.mode,
          blockedReason: "article_material_missing"
        }
      };
    }
...
function hasUsableResearchMaterial(text: string, urls?: string[]) {
