...
export class ArticleFetcher {
  ...
  async fetchDocuments(urls: string[], traceId?: string) {
    const documents: ArticleDocument[] = [];

    for (const sourceUrl of urls) {
      try {
        const document = await this.fetchDocument(sourceUrl, traceId);
  ...
  private async fetchDocument(sourceUrl: string, traceId?: string): Promise<ArticleDocument> {
    let html: string;
    const wechatUrl = isWechatArticleUrl(sourceUrl);
    if (wechatUrl) {
      html = await this.fetchHtmlWithBrowser(sourceUrl);
      await this.writeDebugArtifact(traceId, sourceUrl, "browser_primary_ok", {
      ...
      try {
        ...
        await this.writeDebugArtifact(traceId, sourceUrl, "http_ok", {
          htmlPreview: html.slice(0, 4000),
          htmlLength: html.length
        });
      } catch (error) {
        this.logger.warn("article_fetch.http_failed", {
          sourceUrl,
          error: error instanceof Error ? error.message : String(error)
        });
        await this.writeDebugArtifact(traceId, sourceUrl, "http_failed", {
          error: error instanceof Error ? error.message : String(error)
        });
        html = await this.fetchHtmlWithBrowser(sourceUrl);
        await this.writeDebugArtifact(traceId, sourceUrl, "browser_fallback_ok", {
          htmlPreview: html.slice(0, 4000),
          htmlLength: html.length
        });
      }
    ...
    let scored = selectBestCandidate(sourceUrl, html);
    if (scored.text.length < 300) {
      const browserHtml = await this.tryFetchHtmlWithBrowser(sourceUrl);
      if (browserHtml) {
        const browserScored = selectBestCandidate(sourceUrl, browserHtml);
        if (browserScored.text.length > scored.text.length) {
          scored = browserScored;
          html = browserHtml;
        }
      }
    }
    ...
    await this.writeDebugArtifact(traceId, sourceUrl, "extracted", {
      title,
      excerpt,
      extractionMethod: document.extractionMethod,
      confidence: document.confidence,
      warnings: document.warnings,
      textPreview: document.text.slice(0, 3000),
      textLength: document.text.length
    });
  ...
  private async tryFetchHtmlWithBrowser(sourceUrl: string) {
    try {
      return await this.fetchHtmlWithBrowser(sourceUrl);
    } catch (error) {
      this.logger.warn("article_fetch.browser_failed", {
        sourceUrl,
        error: error instanceof Error ? error.message : String(error)
      });
      return null;
  ...
  private async fetchHtmlWithBrowser(sourceUrl: string) {
    ...
    try {
      ...
      await page.goto(sourceUrl, {
        ...
        timeout: Math.max(this.env.HTTP_TIMEOUT_MS, 30_000)
      });
  ...
  private async writeDebugArtifact(
    ...
    const safeTraceId = traceId && traceId.trim().length > 0 ? traceId : "unknown";
    const dir = path.resolve(this.env.SQLITE_PATH, "..", "fetch-debug", safeTraceId);
    const fileStem = sanitizeFileName(sourceUrl);
    ...
    await writeFile(path.join(dir, `${fileStem}.${stage}.json`), content, "utf8");
  }
}
...
function collectMatches(
  ...
  return Array.from(html.matchAll(matcher))
...
function isWechatArticleUrl(sourceUrl: string) {
  try {
